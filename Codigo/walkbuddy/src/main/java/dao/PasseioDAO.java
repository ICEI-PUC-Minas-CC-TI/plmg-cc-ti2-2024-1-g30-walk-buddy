...
public class PasseioDAO {
	...
	public static String listar(int id) {
		...
		try {
			...
			String sql = "SELECT passeio.id AS passeioid, passeio.estado, passeio.data, passeio.hora, pet.nome AS nomePet, usuario.nome AS nomePasseador "+
				...
				"JOIN pet ON pet.id = passeio_pet.id_pet LEFT JOIN usuario ON usuario.id = passeio.id_passeador WHERE pet.id_dono = " + id + ";";
			ResultSet rs = st.executeQuery(sql);
			lista = "[";
			while(rs.next()) {	            	
	        	json = "{";
	...
	public static String listarPasseador(int id) {
		String lista = "erro";
		String json;
		try {
			Connection conexao = DAO.conectar();
			Statement st = conexao.createStatement();
			String sql = "SELECT passeio.id AS passeioid, passeio.estado, passeio.data, passeio.hora, pet.nome AS nomePet, usuario.nome AS nomeDono "+
				"FROM passeio JOIN passeio_pet ON passeio.id = passeio_pet.id_passeio "+
				"JOIN pet ON pet.id = passeio_pet.id_pet LEFT JOIN usuario ON usuario.id = pet.id_dono WHERE passeio.id_passeador = " + id + ";";
			ResultSet rs = st.executeQuery(sql);
			lista = "[";
			while(rs.next()) {	            	
	        	json = "{";
	    		...
	    		json += "\"hora\": \"" + rs.getString("hora") + "\",";
	    		json += "\"nomePet\": \"" + rs.getString("nomePet") + "\",";
	    		json += "\"nomeDono\": \"" + rs.getString("nomeDono") + "\"";
	    		json += "}";
	        	lista += json + ",";
	        }
			if(lista.charAt(lista.length()-1) == ',')
